...
function beginAfterPick(gs: BloodState, now: number): void {
  ...
  for (const p of gs.players) {
    ...
    switch (effChar(p)) {
      case 'noble':
        p.blood += 12;
        pushLog(gs, 'action', `${pname(p)}【${def.name}】游戏开始：获得 12 血筹`);
        break;
      case 'clerk':
        p.blood += 2;
        pushLog(gs, 'action', `${pname(p)}【${def.name}】游戏开始：额外获得 2 血筹`);
        break;
      case 'actor': {
        const removed: BCard[] = [];
        for (let i = 0; i < 2; i++) {
        ...
        if (removed.length > 0) {
          pushLog(gs, 'action', `${pname(p)}【${def.name}】游戏开始：删除 2 张2（${removed.map(bloodCardText).join(' ')}）`);
        }
        break;
      }
      case 'biker': {
        ...
        pushLog(gs, 'action', `${pname(p)}【${def.name}】游戏开始：删除所有 J/Q/K/A（${royals.length} 张），跳过初始构筑`);
        break;
      }
      case 'imp':
        p.draw = []; // 没有个人牌堆
...
export function bPickChar(gs: BloodState, playerId: string, charId: string, now: number): void {
  ...
  pushLog(gs, 'sys', `${pname(p)} 选择角色：【${def.name}】`);
...
/** 日志用玩家显示名：角色在前、昵称在括号（无角色/失忆时仅昵称） */
function pname(p: BPlayer): string {
  const def = BLOOD_CHAR_BY_ID.get(effChar(p) ?? '');
  return def ? `${def.name}（${p.name}）` : p.name;
}
...
function drawSetupHand(gs: BloodState, p: BPlayer): void {
  ...
  pushLog(gs, 'sys', `${pname(p)} 抽取初始构筑第 ${p.setupRound + 1} 组 8 张牌`);
}
...
function finishDrawPhase(gs: BloodState, now: number): void {
  ...
  for (const p of gs.players) {
    ...
    if (p.swapMalus > 0) {
      ...
      pushLog(gs, 'action', `${pname(p)} 因【餐车投毒】本回合换牌次数 -${malus}（剩 ${swapBase} 次）`);
    }
    if (effChar(p) === 'dungeon') {
      ...
      if (roll >= 3) {
        ...
        pushLog(gs, 'action', `${pname(p)}【地下城主】掷出 ${roll} 点：本回合换牌次数调整为 ${swapBase}`);
      } else {
        p.blood += roll;
        pushLog(gs, 'action', `${pname(p)}【地下城主】掷出 ${roll} 点：获得 ${roll} 血筹（换牌次数仍为 ${swapBase}）`);
      }
    }
...
export function bSetup(gs: BloodState, playerId: string, removedIds: string[], now: number): void {
  ...
  if (removedCards.length > 0) {
    pushLog(gs, 'action', `${pname(p)} 初始构筑删除：${removedCards.map(bloodCardText).join(' ')}`);
  }
...
export function bSwap(gs: BloodState, playerId: string, cardIds: string[], drawCount: number | undefined, now: number): void {
  ...
  if (effChar(p) === 'chef' && chefThrees > 0) {
    p.blood += chefThrees;
    pushLog(gs, 'action', `${pname(p)}【特级大厨】弃置 ${chefThrees} 张3：获得 ${chefThrees} 血筹`);
  }
  // 偶像：一次弃置≥4张获得1血筹
  if (effChar(p) === 'idol' && discardCards.length >= 4) {
    p.blood += 1;
    pushLog(gs, 'action', `${pname(p)}【偶像】一次弃置 ${discardCards.length} 张：获得 1 血筹`);
  }
  ...
  if (effChar(p) === 'streamer' && !p.streamerUsed && discardCards.length >= 2) {
    ...
    if (new Set(ranks).size < ranks.length) {
      p.streamerUsed = true;
      p.blood += 3;
      pushLog(gs, 'action', `${pname(p)}【主播】一次弃置同点数的牌：公示并获得 3 血筹`);
    }
  }
  ...
  if (p.swapLeft <= 0) {
    ...
    if (effChar(p) === 'bartender') {
      p.blood += 1;
      pushLog(gs, 'action', `${pname(p)}【酒保】换牌次数用尽：获得 1 血筹`);
    }
...
function markSwapStopped(gs: BloodState, p: BPlayer): void {
  ...
  if (effChar(p) === 'rose') {
    p.blood += 3;
    pushLog(gs, 'action', `${pname(p)}【白蔷薇】第一位宣告换牌结束：获得 3 血筹`);
  }
}
...
function runTwinSetup(gs: BloodState): void {
  for (const p of gs.players) {
    if (p.charId !== 'twinA' && p.charId !== 'twinB') continue;
    const lensIdx = gs.supply.indexOf('twinLens');
    if (lensIdx < 0) {
      pushLog(gs, 'sys', `${pname(p)}【双生子】黑市牌堆中没有【双生镜片】，跳过`);
      continue;
    }
    gs.supply.splice(lensIdx, 1);
    const candidates = p.discard.filter((c) => !p.chips.some((ch) => ch.on === c.id));
    if (candidates.length === 0) {
      gs.recycle.push('twinLens');
...
function afterSwapEnded(gs: BloodState, p: BPlayer, now: number): void {
  ...
  if (effChar(p) === 'octopus' && p.discard.length > 0) {
    ...
    p.hand.push(...take);
    pushLog(gs, 'action', `${pname(p)}【神作章鱼】从弃牌区随机取回：${take.map(bloodCardText).join(' ')}`);
  }
  ...
  if (effChar(p) === 'undertaker') {
    if (p.undertakerStash.length > 0) {
      ...
      p.undertakerStash = [];
      pushLog(gs, 'action', `${pname(p)}【入殓师】角色牌上的牌置入弃牌区`);
    }
    if (!p.undertakerUsed) {
      p.blood += 2;
      pushLog(gs, 'action', `${pname(p)}【入殓师】本回合未执行特殊换牌：获得 2 血筹`);
    }
  }
...
function tryImpDraw(gs: BloodState, imp: BPlayer, now: number): void {
  ...
  if (sources.length === 0) {
    pushLog(gs, 'action', `${pname(imp)} 无牌可抽（对手抽牌堆均空）`);
    return;
  }
...
function startPlayPhase(gs: BloodState, now: number): void {
  ...
  if (gambler) {
    ...
    pushLog(gs, 'action', `🎲 ${pname(gambler)}【职业赌徒】可猜测本回合的夺魁者`);
  }
  gs.deadline = now + BLOOD_TURN_MS;
}
...
function applyBarrierEffect(gs: BloodState, eff: BarrierEffect, now: number): void {
  ...
  switch (eff.t) {
    case 'violent': {
      ...
      t.removed.push(...top);
      purgeChipsOn(gs, t, new Set(top.map((c) => c.id)));
      pushLog(gs, 'action', `【暴力删除】生效：${pname(t)} 抽牌堆顶 ${top.map(bloodCardText).join(' ')} 被删除`);
      break;
    }
    case 'pinpoint': {
      ...
      if (matches.length === 0) {
        pushLog(gs, 'action', `【定点爆破】${pname(t)} 弃牌堆没有 ${rank} 点的牌，落空`);
      } else {
        const pick = matches[randomInt(0, matches.length)];
        t.discard = t.discard.filter((c) => c.id !== pick.id);
        t.removed.push(pick);
        purgeChipsOn(gs, t, new Set([pick.id]));
        pushLog(gs, 'action', `【定点爆破】生效：${pname(t)} 删除弃牌堆中的 ${bloodCardText(pick)}`);
      }
      break;
    }
    case 'boxRob': {
      ...
      if (myRoll > tRoll) {
        ...
        t.blood -= gain;
        by.blood += gain;
        pushLog(gs, 'action', `【黑厢抢夺】生效：${pname(by)} ${myRoll} 对 ${pname(t)} ${tRoll}，抢夺 ${gain} 血筹`);
      } else {
        pushLog(gs, 'action', `【黑厢抢夺】生效：${pname(by)} ${myRoll} 对 ${pname(t)} ${tRoll}，抢夺失败`);
      }
      break;
    }
    case 'poison': {
      t.swapMalus += 2;
      pushLog(gs, 'action', `【餐车投毒】生效：${pname(t)} 下回合换牌次数 -2`);
      break;
    }
    case 'freeze': {
      t.skipReorg = true;
      pushLog(gs, 'action', `【冻结车厢】生效：${pname(t)} 跳过本回合重整`);
      break;
    }
    case 'amnesia': {
      t.charOffNextRound = true;
      pushLog(gs, 'action', `【暂时失忆】生效：${pname(t)} 下回合技能失效`);
      break;
    }
    case 'signal': {
      if (t.hand.length === 0) {
        pushLog(gs, 'action', `【信号干扰器】生效：${pname(t)} 没有手牌，落空`);
      } else {
        const idx = randomInt(0, t.hand.length);
        const [c] = t.hand.splice(idx, 1);
        t.discard.push(c);
        drawToCap(gs, t);
        pushLog(gs, 'action', `【信号干扰器】生效：${pname(t)} 随机弃置 ${bloodCardText(c)}，抽 1 张牌`);
      }
      break;
    }
    case 'demag': {
...
function openRevealWindow(gs: BloodState, p: BPlayer, now: number): void {
  ...
  if (needSteal) {
    ...
    if (!anyValid) {
      gs.stealPending = null;
      pushLog(gs, 'action', `${pname(p)} 的掠夺效果无合法目标，落空`);
    }
  }
...
export function bSpringUse(gs: BloodState, playerId: string, chipId: string, mod: number, now: number): void {
  ...
  ch.springMod = mod;
  pushLog(gs, 'action', `【弹簧夹层】${pname(p)} 花费 ${Math.abs(mod)} 血筹：${bloodCardText(card)} 临时${mod > 0 ? '+' : ''}${mod} 点（${base} → ${final}）`);
...
export function bRevealChipTarget(
  ...
  if (d.t === 'copy') {
    ...
    pushLog(gs, 'action', `【复制芯片】${pname(p)} 复制了 ${pname(target)} 的【${BLOOD_MARKET_BY_ID.get(defId)?.name}】效果`);
    ...
    targetChip.off = true;
    pushLog(gs, 'action', `【屏蔽器】${pname(p)} 令 ${pname(target)} 的【${BLOOD_MARKET_BY_ID.get(defId)?.name}】本次对决失效`);
  }
  advanceRevealDecision(gs, now);
}
...
export function bDemagPick(gs: BloodState, playerId: string, cardId: string, defId: string, now: number): void {
  ...
  gs.secretPending = null;
  pushLog(gs, 'action', `【消磁枪】${pname(t)} 出牌区的【${def.name}】本次对决失效`);
  nextRevealOrSettle(gs, now);
}
...
export function bPinpointVictimPick(gs: BloodState, playerId: string, cardId: string, now: number): void {
  ...
  t.removed.push(card);
  purgeChipsOn(gs, t, new Set([cardId]));
  const buyer = gs.players.find((x) => x.id === pend.buyerId)!;
  gs.secretPending = null;
  pushLog(gs, 'action', `【定点爆破】${pname(t)} 删除弃牌堆中的 ${bloodCardText(card)}（${rank} 点）`);
...
export function bItemAsk(gs: BloodState, playerId: string, use: boolean, now: number): void {
  ...
  if (!use) {
    if (def) pushLog(gs, 'action', `${pname(p)} 不使用【${def.name}】`);
    gs.secretPending = null;
    advanceItemWindow(gs, now);
    return;
  }
  ...
  switch (def.effect.k) {
    case 'signalJamFx':
      ...
      pushLog(gs, 'action', `${pname(p)} 使用【信号干扰器】：请选择一位玩家随机弃 1 抽 1`);
    ...
    case 'secretNoteFx': {
      ...
      p.hand.push(...drawn);
      gs.recycle.push(item.def);
      pushLog(gs, 'action', `${pname(p)} 使用【皮下密信】：支付 2 血筹，抽 3 张牌`);
      gs.secretPending = null;
      advanceItemWindow(gs, now);
      return;
    }
    case 'eraserFx':
      ...
      pushLog(gs, 'action', `${pname(p)} 使用【魔术橡皮】：请宣称一种牌型`);
      return;
    case 'irisGambleFx':
      gs.deadline = now + BLOOD_TURN_MS; // 转入子交互，重置决策时限
      gs.secretPending = { seat: p.id, kind: 'irisGuess', defId: item.def };
      pushLog(gs, 'action', `${pname(p)} 使用【赌徒虹膜】：请选择竞猜目标与牌型`);
      return;
    case 'loudspeakerFx':
      gs.recycle.push(item.def);
      p.claimedWin = true;
      gs.announce = { defId: item.def, buyerSeat: p.seat, at: now };
      pushLog(gs, 'action', `${pname(p)} 使用【广播喇叭】：宣称本回合将夺魁！`);
      gs.secretPending = null;
      advanceItemWindow(gs, now);
      return;
    case 'dealerLicense':
      ...
      pushLog(gs, 'action', `${pname(p)} 使用【荷官证】：本次对决先比总点数，平局再比牌型（对决前宣告）`);
      gs.secretPending = null;
      advanceItemWindow(gs, now);
      return;
    default:
...
export function bUseItem(gs: BloodState, playerId: string, itemId: string | null, now: number): void {
  ...
  if (itemId == null) {
    pushLog(gs, 'action', `${pname(p)} 宣告完毕`);
    nextRevealOrSettle(gs, now);
    return;
  }
...
function settle(gs: BloodState, now: number): void {
  ...
  if (curHolder && curHolder !== winner && effChar(curHolder) === 'sunwu') {
    pushLog(gs, 'action', `${pname(curHolder)}【江东之主】的【临时特权证】不可被夺走`);
  } else {
    for (const p of gs.players) p.privilege = false;
    ...
    for (const ch of p.chips.filter((cc) => p.play.some((card) => card.id === cc.on) && !cc.off)) {
      ...
      for (const eff of chipEffectsFor(p, ch)) {
        if (eff.k === 'settleWin' && p === winner) {
          ...
          p.blood += g;
          pushLog(gs, 'action', `${pname(p)} 的【${def.name}】发动：魁首获得 ${g} 血筹`);
        } else if (eff.k === 'settleLose' && p !== winner) {
          const g = settleGainBlood(gs, p, eff.blood);
          p.blood += g;
          pushLog(gs, 'action', `${pname(p)} 的【${def.name}】发动：战败获得 ${g} 血筹`);
        }
      }
    }
  }
  ...
  if (gs.gamblerGuess) {
    ...
    if (guesser && gs.gamblerGuess.seat === winner.seat) {
      ...
      guesser.blood += gain;
      pushLog(gs, 'action', `${pname(guesser)}【职业赌徒】猜中夺魁者：获得 ${gain} 血筹`);
    } else if (guesser) {
      pushLog(gs, 'action', `${pname(guesser)}【职业赌徒】竞猜落空`);
    }
    gs.gamblerGuess = null;
  }
  ...
  for (const p of gs.players) {
    ...
    if (took > 0) pushLog(gs, 'action', `${pname(p)}【无业游民】出牌区含对手的牌：收取 ${took} 血筹`);
  }
  ...
  if (imp) {
    ...
    if (has) {
      imp.tickets += 1;
      pushLog(gs, 'action', `${pname(imp)}【捣蛋鬼】出牌区含夺魁者的牌：获得 1 车票`);
    }
  }
    ...
    for (const ch of p.chips.filter((cc) => p.play.some((card) => card.id === cc.on) && !cc.off)) {
      ...
      for (const eff of chipEffectsFor(p, ch)) {
        if (eff.k === 'settleWinTicket' && p === winner) {
          ...
          pushLog(gs, 'action', `${pname(p)} 的【${def.name}】发动：魁首获得 ${eff.tickets} 车票`);
        }
      }
    }
  }
  ...
  if (gs.irisGuess) {
    ...
    if (guesser && target && row) {
      ...
      if (row.cat === cat) {
        ...
        row.gainTickets -= dec;
        target.tickets -= dec;
        pushLog(gs, 'action', `【赌徒虹膜】${pname(guesser)} 猜中 ${pname(target)} 的【${guessed}】：获得 3 血筹，${pname(target)} 本回合车票 -${dec}`);
      } else {
        pushLog(gs, 'action', `【赌徒虹膜】${pname(guesser)} 竞猜落空（猜【${guessed}】，实际【${actual}】）`);
      }
    }
    gs.irisGuess = null;
  }
  for (const r of rows) {
    ...
    if (p === winner) {
      ...
      p.blood += gain;
      pushLog(gs, 'action', `【广播喇叭】${pname(p)} 宣称成功夺魁：获得 ${gain} 血筹`);
    } else {
      p.skipBuyRemove = true;
      p.skipReorg = true;
      pushLog(gs, 'action', `【广播喇叭】${pname(p)} 宣称失败：跳过本回合购买、删牌、重整`);
    }
  }
    ...
    switch (p.charId) {
      case 'miner':
        if (allSuitsMatch(gs, p, 'black')) {
          ...
          pushLog(gs, 'action', `${pname(p)}【矿工】打出的牌均为黑色：获得 3 血筹`);
        }
        break;
      case 'painter': {
        ...
        if (gain > 0) {
          ...
          pushLog(gs, 'action', `${pname(p)}【画家】出牌区存在 ${n} 种花色：获得 ${gain} 血筹`);
        }
        break;
      }
      case 'chef': {
        ...
        if (threes > 0) {
          ...
          pushLog(gs, 'action', `${pname(p)}【特级大厨】打出 ${threes} 张3：获得 ${threes} 血筹`);
        }
        break;
      }
      case 'screenwriter': {
        ...
        if (ev.pips === 50) {
          ...
          pushLog(gs, 'action', `${pname(p)}【编剧】总点数恰好 50：获得 5 血筹`);
        } else {
          p.skipBuyRemove = true;
          pushLog(gs, 'action', `${pname(p)}【编剧】总点数 ${ev.pips} ≠ 50：跳过本回合购买与删牌阶段`);
        }
        break;
      }
      case 'princess': {
        ...
        if (dark && triggered) {
          p.blood += settleGainBlood(gs, p, 3);
          pushLog(gs, 'action', `${pname(p)}【双重人格公主·常时】打出的牌均为黑色：获得 3 血筹`);
        } else if (!dark && triggered) {
          r.gainTickets += 1;
          p.tickets += 1;
          pushLog(gs, 'action', `${pname(p)}【双重人格公主·躁狂】打出的牌均为红色：获得 1 车票`);
        }
      ...
      case 'samurai':
        if (r.gainTickets > 0) {
          ...
          pushLog(gs, 'action', `${pname(p)}【武士】额外获得 ${r.gainTickets} 血筹（等量于本回合车票）`);
        }
        break;
      case 'sunwu':
        if (p === winner) {
          p.blood += settleGainBlood(gs, p, 2);
          pushLog(gs, 'action', `${pname(p)}【江东之主】夺魁：获得 2 血筹`);
        }
        break;
    }
  }
  ...
  if (gs.bomberX != null && gs.bomberX > 0) {
    for (const p of gs.players) {
      ...
      purgeChipsOn(gs, p, new Set(deleted.map((c) => c.id)));
      if (deleted.length > 0) {
        pushLog(gs, 'action', `💣 ${pname(p)}${effChar(p) === 'bomber' ? '【炸弹客】' : '（炸弹客宣告）'}随机删除本回合打出的牌：${deleted.map(bloodCardText).join(' ')}`);
      }
    }
  }
...
function processSettleQueue(gs: BloodState, now: number): void {
  ...
  while (gs.settleQueue.length > 0) {
    ...
    if (next.kind === 'succubusSteal') {
      ...
      if (targets.length === 0) {
        p.blood += amount;
        pushLog(gs, 'action', `${pname(p)}【魅魔】无法抢夺：直接获得 ${amount} 血筹`);
        continue;
      }
...
export function bShowdownDone(gs: BloodState, playerId: string, now: number): void {
  ...
  p.sdSeen = true;
  pushLog(gs, 'action', `${pname(p)} 已确认对决展示`);
...
function startBuyPhase(gs: BloodState, now: number): void {
  ...
  gs.deadline = now + BLOOD_TURN_MS;
  pushLog(gs, 'hand', `购买阶段：从 ${pname(winner)} 开始顺时针购买`);
...
export function bBuy(
  ...
  if (mascotDeal) {
    pushLog(gs, 'action', `${pname(p)}【吉祥物】首次购买优惠：${def.cost} → ${cost} 血筹`);
  }
  if (smuggled && gs.smugglerMark!.by === p.id) {
    pushLog(gs, 'action', `${pname(p)}【走私客】购买自己标记的牌：价格 -2`);
  }
  const bonusTxt = bonusTaken > 0 ? `，连同栏位上的 ${bonusTaken} 血筹一并取走` : '';
  pushLog(gs, 'action', `${pname(p)} 支付 ${cost} 血筹购买【${def.name}】${bonusTxt}`);
...
function processMarketDef(
  ...
  switch (def.effect.k) {
    case 'rollDice': {
      const roll = randomInt(1, 7);
      p.blood += roll;
      pushLog(gs, 'action', `【${def.name}】发动：${pname(p)} 掷出 ${roll} 点，获得 ${roll} 血筹`);
    ...
    case 'privilegeBonus': {
      if (p.privilege) {
        ...
        pushLog(gs, 'action', `【${def.name}】发动：${pname(p)} 持有特权证，获得 ${def.effect.blood} 血筹`);
      } else {
    ...
    case 'closingGift': {
      ...
      p.lastAction = '跳过购买';
      gs.recycle.push(def.id);
      pushLog(gs, 'action', `【${def.name}】发动：${pname(p)} 获得 ${def.effect.blood} 血筹，本回合不再购买`);
      afterMarketResolved(gs, p, false);
      return;
    }
    case 'bloodShare': {
    ...
    case 'stealPrivilege': {
      // 鬼手探囊：夺得更衣室大权——临时特权证归你（江东之主持有期间不可夺）
      const holder = gs.players.find((o) => o.privilege);
      if (holder && effChar(holder) === 'sunwu') {
        gs.recycle.push(def.id);
        pushLog(gs, 'action', `【${def.name}】发动失败：${pname(holder)}【江东之主】的特权证不可被夺走`);
      } else {
    ...
    case 'preciseDelDraw': {
      ...
      pushLog(gs, 'action', `【${def.name}】发动：${pname(p)} 抽 3 张牌，选择 0-2 张删除，其余弃置`);
      return;
    }
    case 'pullChipGain': {
      gs.recycle.push(def.id);
    ...
    case 'sharedInfoFx': {
      ...
      pushLog(gs, 'action', `【${def.name}】发动：${pname(p)} 可删除至多 2 张牌，随后每位对手可删除 1 张`);
      return;
    }
  }
}
...
function afterMarketResolved(gs: BloodState, p: BPlayer, refreshExtraBuy: boolean): void {
  ...
  if (p.baristaPending) {
    ...
    if (topId) {
      const topDef = BLOOD_MARKET_BY_ID.get(topId)!;
      pushLog(gs, 'action', `☕ ${pname(p)}【咖啡师】免费获得黑市牌堆顶的【${topDef.name}】`);
      gs.announce = { defId: topId, buyerSeat: p.seat, at: Date.now() };
...
function insertChip(gs: BloodState, p: BPlayer, chipId: string, defId: string, cardId: string): void {
  ...
  pushLog(gs, 'action', `${pname(p)} 将【${def.name}】插入 ${bloodCardText(card)}`);
}
...
export function bPassBuy(gs: BloodState, playerId: string, now: number): void {
  ...
  p.buyPassed = true;
  p.lastAction = '跳过购买';
  pushLog(gs, 'action', `${pname(p)} 跳过购买（本阶段不可再买）`);
...
function endBuy(gs: BloodState, now: number): void {
  ...
  for (const p of gs.players) {
    if (effChar(p) === 'stockholder' && p.blood === 0) {
      p.blood += 3;
      pushLog(gs, 'action', `${pname(p)}【股民】购买阶段结束剩余 0 血筹：获得 3 血筹`);
    }
    if (effChar(p) === 'wei' && p.boughtAny) {
      p.blood += 2;
      pushLog(gs, 'action', `${pname(p)}【魏王】本回合购买过黑市牌：获得 2 血筹`);
    }
  }
  gs.phase = 'remove';
  for (const p of gs.players) {
    ...
    if (effChar(p) === 'maid') {
      p.removeDone = true;
      pushLog(gs, 'action', `${pname(p)}【女仆】跳过删牌阶段`);
...
export function bRemove(gs: BloodState, playerId: string, cardIds: string[], now: number): void {
  ...
  if (cards.length > 0) {
    pushLog(gs, 'action', `${pname(p)} 删除 ${cards.length} 张（${cost > 0 ? `支付 ${cost} 血筹` : '免费'}）：${cards.map(bloodCardText).join(' ')}`);
  } else {
...
/** 金科玉律10：被删除的牌连同其强化芯片一起进删牌区（芯片实例移出游戏，计入回收站） */
function purgeChipsOn(gs: BloodState, p: BPlayer, cardIds: Set<string>): void {
  const removed = p.chips.filter((ch) => cardIds.has(ch.on));
  if (removed.length === 0) return;
  p.chips = p.chips.filter((ch) => !cardIds.has(ch.on));
  for (const ch of removed) gs.recycle.push(ch.def);
  pushLog(
    gs,
    'sys',
    `⛓️ 被删除的牌上的强化芯片随之移出游戏：${removed.map((ch) => `【${BLOOD_MARKET_BY_ID.get(ch.def)?.name ?? '?'}】`).join('、')}`,
  );
}
...
function startReorg(gs: BloodState, now: number): void {
  ...
  for (const p of gs.players) {
    if (p.skipReorg || effChar(p) === 'imp') {
      p.reorgDone = true;
      pushLog(gs, 'action', `${pname(p)} 跳过重整阶段${effChar(p) === 'imp' ? '【捣蛋鬼】' : ''}`);
    }
  }
  gs.deadline = now + BLOOD_TURN_MS;
...
export function bReorg(
  ...
  if (p.skipReorg) {
    p.reorgDone = true;
    p.lastAction = '跳过重整';
    pushLog(gs, 'action', `${pname(p)} 跳过重整阶段`);
    if (allDone(gs, (x) => x.reorgDone)) finishReorg(gs, now);
    return;
  }
  if (choice === 'reshuffle') {
    ...
    if (effChar(p) === 'inspector') {
      p.blood += 1;
      pushLog(gs, 'action', `${pname(p)}【质检员】重洗牌库：获得 1 血筹`);
    }
  } else {
    p.blood += 2;
    ...
    if (effChar(p) === 'laundry') {
      p.blood += 2;
      pushLog(gs, 'action', `${pname(p)}【洗衣房店主】选择不重洗：额外获得 2 血筹`);
    }
    pushLog(gs, 'action', `${pname(p)} 获得 2 血筹`);
    ...
    if (effChar(p) === 'inspector' && pickCardId) {
      ...
      if (card) {
        ...
        p.draw.push(card);
        pushLog(gs, 'action', `${pname(p)}【质检员】公示 ${bloodCardText(card)} 并放到抽牌堆顶`);
      }
    }
  }
  ...
  if (effChar(p) === 'clerk') {
    p.blood += 2;
    pushLog(gs, 'action', `${pname(p)}【银行职员】重整阶段获得 2 血筹`);
  }
  p.reorgDone = true;
  ...
  switch (gs.phase) {
    ...
    case 'setup': {
      ...
      if (pend?.kind === 'mynameSet') {
        ...
        gs.mynameText = '神秘牌型';
        gs.secretPending = null;
        pushLog(gs, 'action', `${pname(p)}【我的名字？】超时托管：将「一对」命名为「神秘牌型」`);
        processStartupQueue(gs);
        return true;
      }
...
function resolveSwapEndOnTimeout(gs: BloodState, p: BPlayer, now: number): void {
  ...
  switch (pend.kind) {
    case 'itemAsk': {
      const def = BLOOD_MARKET_BY_ID.get(pend.defId ?? '');
      pushLog(gs, 'action', `${pname(p)} 超时：不使用【${def?.name ?? '道具'}】`);
      gs.secretPending = null;
      advanceItemWindow(gs, now);
      return;
    }
    ...
    case 'bomberClaim':
      pushLog(gs, 'action', `${pname(p)}【炸弹客】宣告超时：本回合不发动`);
      gs.bomberX = 0;
      break;
    case 'curseTake':
      pushLog(gs, 'action', `${pname(p)}【咒术师】取回超时：藏牌保留在角色牌下`);
      break;
    case 'generalChoice':
      pushLog(gs, 'action', `${pname(p)}【将军】选择超时：视为放弃`);
      break;
    case 'godPeek':
      pushLog(gs, 'action', `${pname(p)}【赌神】选择超时：视为放弃`);
      break;
    case 'vagrantDraw':
      pushLog(gs, 'action', `${pname(p)}【无业游民】抽牌超时：视为放弃`);
      break;
    case 'ceoGive':
...
function resolveSettleOnTimeout(gs: BloodState, p: BPlayer, now: number): void {
  ...
  if (pend.kind === 'succubusSteal') {
    ...
    if (targets.length > 0) {
      ...
      t.blood -= pay;
      p.blood += pay;
      pushLog(gs, 'action', `${pname(p)}【魅魔】托管抢夺 ${pname(t)}：获得 ${pay} 血筹`);
    } else {
      p.blood += amount;
      pushLog(gs, 'action', `${pname(p)}【魅魔】托管：直接获得 ${amount} 血筹`);
    }
    processSettleQueue(gs, now);
    return;
  }
  if (pend.kind === 'scalperDeal') {
    pushLog(gs, 'action', `${pname(p)}【票贩子】超时：放弃强购`);
    gs.secretPending = null;
    processSettleQueue(gs, now);
    return;
  }
  if (pend.kind === 'fryerDel') {
    pushLog(gs, 'action', `${pname(p)}【炸鸡店老板】删牌超时：结束`);
    gs.secretPending = null;
    processSettleQueue(gs, now);
    void now;
  }
...
export function bSecretTarget(gs: BloodState, playerId: string, seat: number, now: number): void {
  ...
  // 暂时失忆卡面为“一位玩家”：允许指定自己（跳过屏障询问）
  if (pend.kind === 'amnesiaTarget' && t.id === p.id) {
    t.charOffNextRound = true;
    gs.recycle.push(pend.defId ?? 'amnesia');
    gs.secretPending = null;
    pushLog(gs, 'action', `【暂时失忆】${pname(p)} 使自己的技能下回合失效`);
    afterMarketResolved(gs, p, false);
    return;
  }
  ...
  switch (pend.kind) {
    case 'poisonTarget': {
      ...
      pushLog(gs, 'action', `【餐车投毒】${pname(p)} 毒害 ${pname(t)}：下回合换牌次数 -2`);
      finish();
      return;
    }
    case 'freezeTarget': {
      ...
      pushLog(gs, 'action', `【冻结车厢】${pname(t)} 跳过本回合重整阶段`);
      finish();
      return;
    }
    case 'amnesiaTarget': {
      t.charOffNextRound = true;
      gs.recycle.push(pend.defId ?? 'amnesia');
      pushLog(gs, 'action', `【暂时失忆】${pname(t)} 的角色技能在下回合失效`);
      finish();
      return;
    }
    case 'boxRobTarget': {
      ...
      if (myRoll > tRoll) {
        ...
        t.blood -= gain;
        p.blood += gain;
        pushLog(gs, 'action', `【黑厢抢夺】${pname(p)} 掷出 ${myRoll}，${pname(t)} 掷出 ${tRoll}：抢夺 ${gain} 血筹`);
      } else {
        pushLog(gs, 'action', `【黑厢抢夺】${pname(p)} 掷出 ${myRoll}，${pname(t)} 掷出 ${tRoll}：抢夺失败，无事发生`);
      }
      finish();
      return;
    }
    case 'signalTarget': {
      if (t.hand.length === 0) {
        pushLog(gs, 'action', `【信号干扰器】${pname(t)} 没有手牌，效果落空`);
      } else {
        const idx = randomInt(0, t.hand.length);
        const [c] = t.hand.splice(idx, 1);
        t.discard.push(c);
        drawToCap(gs, t);
        pushLog(gs, 'action', `【信号干扰器】${pname(t)} 随机弃置 ${bloodCardText(c)}，并抽 1 张牌`);
      }
      finish();
      return;
    }
    case 'demagTarget': {
      ...
      if (chips.length === 0) {
        gs.recycle.push(pend.defId ?? 'demag');
        gs.secretPending = null;
        pushLog(gs, 'action', `【消磁枪】${pname(t)} 的出牌区没有强化芯片，效果落空`);
        nextRevealOrSettle(gs, now);
        return;
      }
      // 使用者自选目标玩家的具体芯片
...
export function bEraserClaim(gs: BloodState, playerId: string, cat: number, now: number): void {
  ...
  gs.secretPending = null;
  pushLog(gs, 'action', `【魔术橡皮】${pname(p)} 宣告：本回合【${catName(cat)}】视为高牌`);
  advanceItemWindow(gs, now);
}
...
export function bIrisGuess(gs: BloodState, playerId: string, seat: number, cat: number, now: number): void {
  ...
  gs.secretPending = null;
  pushLog(gs, 'action', `【赌徒虹膜】${pname(p)} 竞猜 ${pname(t)} 的牌型为【${catName(cat)}】`);
  advanceItemWindow(gs, now);
}
...
export function bPinpoint(gs: BloodState, playerId: string, seat: number, rank: number, now: number): void {
  ...
  pushLog(gs, 'action', `【定点爆破】${pname(t)} 须从弃牌堆选择一张 ${rank} 点的牌删除`);
  gs.deadline = now + BLOOD_TURN_MS;
}
...
export function bPreciseDel(gs: BloodState, playerId: string, cardIds: string[], now: number): void {
  ...
  purgeChipsOn(gs, p, new Set(picked.map((c) => c.id)));
  ...
  pushLog(
    ...
    'action',
    picked.length > 0
      ? `【精准删除】${pname(p)} 删除 ${picked.map(bloodCardText).join(' ')}，其余 ${rest.length} 张弃置`
      : `【精准删除】${pname(p)} 未删除任何牌，3 张全部弃置`,
  );
...
export function bPullChip(gs: BloodState, playerId: string, cardId: string, now: number): void {
  ...
  p.blood += 4;
  pushLog(gs, 'action', `【拔除芯片】${pname(p)} 拔除 ${bloodCardText(card)} 上的【${def.name}】：获得 4 血筹`);
  gs.secretPending = null;
  afterMarketResolved(gs, p, false);
}
  ...
  switch (pend.kind) {
    ...
    case 'pinpointVictim': {
      ...
      if (matches.length === 0) {
        ...
        pushLog(gs, 'action', `【定点爆破】托管：${pname(p)} 删除弃牌堆中的 ${bloodCardText(pickCard)}`);
      }
      const buyer = gs.players.find((x) => x.id === pend.buyerId)!;
      afterMarketResolved(gs, buyer, false);
      return;
    ...
    /* ---- 购买阶段前的角色互动托管 ---- */
    case 'pirateRob':
      pushLog(gs, 'action', `${pname(p)}【海盗】超时：放弃抢劫`);
      gs.secretPending = null;
      processPreBuyQueue(gs, now);
      return;
    case 'pirateDecide': {
    ...
    case 'smugglerMark':
      pushLog(gs, 'action', `${pname(p)}【走私客】超时：本回合不标记`);
      gs.secretPending = null;
      processPreBuyQueue(gs, now);
      return;
    case 'auctionPick': {
      // 托管：不发动拍卖，两张牌放回原处
      pushLog(gs, 'action', `${pname(p)}【瞎掰帝】超时：不发动拍卖`);
      gs.secretPending = null;
      processPreBuyQueue(gs, now);
      return;
    }
    ...
    case 'impRedeem':
      pushLog(gs, 'action', `${pname(p)} 超时：不向捣蛋鬼赎回`);
      gs.secretPending = null;
      processPreBuyQueue(gs, now);
      return;
  }
...
export function bResign(gs: BloodState, playerId: string, now: number): void {
  ...
  const sunwuHolder = gs.players.find((x) => x.charId === 'sunwu' && x.id !== playerId);
  for (const o of gs.players) o.privilege = false;
  if (sunwuHolder) {
    sunwuHolder.privilege = true;
    gs.privilegeSeat = sunwuHolder.seat;
  } else {
    winner.privilege = true;
    gs.privilegeSeat = winner.seat;
  }
...
export function bMynameSet(gs: BloodState, playerId: string, cat: number, name: string, now: number): void {
  ...
  gs.secretPending = null;
  pushLog(gs, 'action', `${pname(p)}【我的名字？】将「${catName(cat)}」命名为「${trimmed}」：此后任何人打出该牌型，其获得 2 血筹`);
...
export function bHackerSetup(gs: BloodState, playerId: string, removedIds: string[], now: number): void {
  ...
  p.setupRound = 2;
  gs.secretPending = null;
  pushLog(gs, 'action', `${pname(p)}【黑客】初始构筑：从全牌库删除 8 张（${cards.map(bloodCardText).join(' ')}）`);
  afterStartupResolved(gs, now);
}
...
export function bDetectivePick(
  ...
  if (mode === 'skip') {
    ...
    pushLog(gs, 'action', `${pname(p)}【私家侦探】公示 ${cards.map(bloodCardText).join(' ')} 并放到抽牌堆底`);
  }
  processPreDrawQueue(gs, now);
}
...
export function bFacelessPick(gs: BloodState, playerId: string, charId: string, now: number): void {
  ...
  gs.secretPending = null;
  const def = BLOOD_CHAR_BY_ID.get(charId)!;
  pushLog(gs, 'action', `🎭 ${pname(p)}【无面人】获得临时技能：【${def.name}】（持续至下个抽牌阶段前）`);
  processPreDrawQueue(gs, now);
}
...
export function bFacelessConvert(gs: BloodState, playerId: string, now: number): void {
  ...
  p.tempChar = null;
  p.facelessDone = true;
  pushLog(gs, 'action', `🎭 ${pname(p)}【无面人】永久转化为【${def.name}】（不可逆转）`);
}
...
export function bBomberClaim(gs: BloodState, playerId: string, x: number, now: number): void {
  ...
  if (x > 0) {
    p.blood += x;
    pushLog(gs, 'action', `💣 ${pname(p)}【炸弹客】宣告 X=${x}：获得 ${x} 血筹（结算时将随机删牌）`);
  } else {
    pushLog(gs, 'action', `💣 ${pname(p)}【炸弹客】宣告 X=0：无事发生`);
  }
  checkSwapEnd(gs, now);
}
...
export function bStudentDump(gs: BloodState, playerId: string, accept: boolean, cardId: string | undefined, now: number): void {
  ...
  if (pend.kind === 'studentDump') {
    gs.secretPending = null;
    if (!accept) {
      pushLog(gs, 'action', `${pname(p)}【高中生】保留出牌区`);
      afterPlayHookResolved(gs, now);
      return;
    }
  ...
  if (pend.kind === 'studentRemove') {
    ...
    if (accept && cardId) {
      ...
      p.removed.push(card);
      purgeChipsOn(gs, p, new Set([cardId]));
      pushLog(gs, 'action', `${pname(p)}【高中生】执行删牌：删除 ${bloodCardText(card)}（支付 2 血筹）`);
      gainChefDeleteThrees(gs, p, [card]);
      checkLiuWin(gs, p, now);
    } else {
      pushLog(gs, 'action', `${pname(p)}【高中生】放弃删牌`);
    }
    afterPlayHookResolved(gs, now);
    return;
  }
...
export function bDesignerDiscard(gs: BloodState, playerId: string, cardIds: string[], now: number): void {
  ...
  p.blood += gain;
  gs.secretPending = null;
  pushLog(gs, 'action', `🎲 ${pname(p)}【桌游设计师】弃置出牌区 ${cards.length} 张：获得 ${gain} 血筹`);
  afterPlayHookResolved(gs, now);
}
...
export function bAgentDecide(gs: BloodState, playerId: string, accept: boolean, now: number): void {
  ...
  if (accept) {
    ...
    t.blood -= pay;
    agent.blood += pay;
    pushLog(gs, 'action', `🤝 ${pname(t)} 拒绝交换：支付 ${pay} 血筹给 ${pname(agent)}【特工】`);
  }
  afterPlayHookResolved(gs, now);
}
...
export function bBlufferDeclare(
  ...
  pushLog(gs, 'action', `🗣️ ${pname(p)}【瞎掰王】宣告出牌区为：${txt}（等待质疑）`);
...
export function bBlufferChallenge(gs: BloodState, playerId: string, challenge: boolean, now: number): void {
  ...
  pushLog(gs, 'action', challenge ? `🗣️ ${pname(p)} 质疑【瞎掰王】的宣告` : `👀 ${pname(p)} 不质疑`);
  const next = pend.oppQueue?.shift();
  if (next) {
  ...
  if (bl.challengers.length > 0) {
    ...
    if (consistent) {
      ...
      pushLog(gs, 'action', `🗣️ 宣告与出牌区一致：质疑者各支付 1 血筹给 ${pname(bluffer)}【瞎掰王】`);
    } else {
      for (const cid of bl.challengers) {
        const c = gs.players.find((x) => x.id === cid)!;
        c.blood += 1;
    ...
    pushLog(gs, 'action', `🗣️ 无人质疑：按 ${pname(bluffer)} 的宣告进行对决（实际牌弃置）`);
  }
  afterPlayHookResolved(gs, now);
}
...
export function bSuccubusSteal(gs: BloodState, playerId: string, seat: number, now: number): void {
  ...
  t.blood -= pay;
  p.blood += pay;
  pushLog(gs, 'action', `${pname(p)}【魅魔】抢夺 ${pname(t)}：获得 ${pay} 血筹`);
  processSettleQueue(gs, now);
}
...
export function bScalperDeal(gs: BloodState, playerId: string, accept: boolean, now: number): void {
  ...
  winner.tickets -= 1;
  p.tickets += 1;
  pushLog(gs, 'action', `🎫 ${pname(p)}【票贩子】支付 3 血筹向 ${pname(winner)} 强购 1 车票`);
...
export function bFryerDraw(gs: BloodState, playerId: string, now: number): void {
  ...
  p.hand.push(...drawn);
  pushLog(gs, 'action', `🍗 ${pname(p)}【炸鸡店老板】支付 1 血筹抽 1 张牌`);
}
...
export function bCurseHide(gs: BloodState, playerId: string, cardId: string, now: number): void {
  ...
  p.hand.push(...drawn);
  p.blood += 1;
  pushLog(gs, 'action', `✨ ${pname(p)}【咒术师】藏入 1 张【5】并抽 1 张牌：获得 1 血筹（共藏 ${p.curseStash.length} 张）`);
}
...
export function bCurseTake(gs: BloodState, playerId: string, cardIds: string[], now: number): void {
  ...
  p.hand.push(...taken);
  gs.secretPending = null;
  pushLog(gs, 'action', `✨ ${pname(p)}【咒术师】取回 ${taken.length} 张【5】入手牌`);
  checkSwapEnd(gs, now);
}
...
export function bUndertakerSwap(gs: BloodState, playerId: string, cardIds: string[], now: number): void {
  ...
  pushLog(
    gs,
    'action',
    `⚰️ ${pname(p)}【入殓师】特殊换牌：置 ${cards.length} 张于角色牌上，从弃牌区随机取回 ${taken.map(bloodCardText).join(' ') || '（弃牌区为空）'}`,
  );
  if (p.swapLeft <= 0) {
    p.swapDone = true;
    markSwapStopped(gs, p);
...
export function bGodPeekChoice(gs: BloodState, playerId: string, mode: 'extra' | 'blood', now: number): void {
  ...
  if (mode === 'extra') {
    ...
    p.lastAction = '额外换牌';
    pushLog(gs, 'action', `🎲 ${pname(p)}【赌神】看牌后选择：额外进行一次换牌`);
  } else {
    p.blood += 1;
    pushLog(gs, 'action', `🎲 ${pname(p)}【赌神】看牌后选择：获得 1 血筹`);
  }
  checkSwapEnd(gs, now);
}
...
export function bGeneralChoice(
  ...
  if (mode === 'gift') {
    ...
    if (t.hand.length > 0) {
      ...
      const [c] = t.hand.splice(idx, 1);
      t.discard.push(c);
      pushLog(gs, 'action', `🎖️ ${pname(p)}【将军】令 ${pname(t)} 随机弃置 ${bloodCardText(c)}`);
    }
    const drawn = drawN(gs, t, 1);
    t.hand.push(...drawn);
    pushLog(gs, 'action', `🎖️ ${pname(t)} 摸 1 张牌`);
  } else if (mode === 'extra') {
    p.swapDone = false;
    p.swapLeft += 1;
    p.extraSwapProtected = true;
    p.lastAction = '额外换牌';
    pushLog(gs, 'action', `🎖️ ${pname(p)}【将军】选择额外进行一次换牌`);
  } else {
    pushLog(gs, 'action', `🎖️ ${pname(p)}【将军】放弃发动`);
  }
  checkSwapEnd(gs, now);
}
...
export function bSmugglerMark(gs: BloodState, playerId: string, slot: number, now: number): void {
  ...
  gs.secretPending = null;
  if (slot < 0) {
    pushLog(gs, 'action', `🚚 ${pname(p)}【走私客】本回合不标记`);
    processPreBuyQueue(gs, now);
    return;
  }
  const ms = gs.market[slot];
...
export function bPirateRob(gs: BloodState, playerId: string, seat: number, now: number): void {
  ...
  gs.secretPending = null;
  if (seat < 0) {
    pushLog(gs, 'action', `🏴‍☠️ ${pname(p)}【海盗】放弃抢劫`);
    processPreBuyQueue(gs, now);
    return;
  }
  const t = bySeat(gs, seat);
  ...
  gs.deadline = now + BLOOD_TURN_MS;
  pushLog(gs, 'action', `🏴‍☠️ ${pname(p)}【海盗】抢劫 ${pname(t)}：其须选择【放弃】或【抵抗】`);
}
...
export function bPirateDecide(gs: BloodState, playerId: string, resist: boolean, now: number): void {
  ...
  if (!resist) {
    ...
    t.blood -= pay;
    pirate.blood += pay;
    pushLog(gs, 'action', `🏴‍☠️ ${pname(t)} 选择【放弃】：交给 ${pname(pirate)}【海盗】 ${pay} 血筹`);
  } else {
    ...
    if (pRoll > tRoll) {
      ...
      t.blood -= gain;
      pirate.blood += gain;
      pushLog(gs, 'action', `🏴‍☠️ ${pname(t)} 选择【抵抗】：${pname(pirate)} 掷出 ${pRoll}，${pname(t)} 掷出 ${tRoll}，${pname(pirate)}【海盗】抢夺 ${gain} 血筹`);
    } else {
      pushLog(gs, 'action', `🏴‍☠️ ${pname(t)} 选择【抵抗】：${pname(pirate)} 掷出 ${pRoll}，${pname(t)} 掷出 ${tRoll}，抢劫失败`);
    }
  }
  processPreBuyQueue(gs, now);
}
...
export function bAuctionPick(gs: BloodState, playerId: string, idx: number, now: number): void {
  ...
  gs.deadline = now + BLOOD_TURN_MS;
  pushLog(gs, 'action', `🔨 ${pname(p)}【瞎掰帝】暗置一张牌开始叫价（玩家不可查看），从其开始顺时针一轮`);
}
...
export function bAuctionBid(gs: BloodState, playerId: string, amount: number, now: number): void {
  ...
  if (amount > 0) {
    ...
    pushLog(gs, 'action', `🔨 ${pname(p)} 叫价 ${amount} 血筹`);
  } else {
    pushLog(gs, 'action', `🔨 ${pname(p)} 不叫价`);
  }
...
export function bBuySeer(gs: BloodState, playerId: string, idx: number, now: number): void {
  ...
  pushLog(gs, 'action', `🔮 ${pname(p)}【窥天师】以 ${cost} 血筹购入天意【${def.name}】`);
  if (gs.seerZone.length === 0) {
    p.tickets = 0;
    pushLog(gs, 'action', `🌀 天意均被购入：${pname(p)} 被天意侵蚀，车票归 0！`);
  }
...
export function bImpRedeem(gs: BloodState, playerId: string, accept: boolean, now: number): void {
  ...
  pushLog(gs, 'action', `💰 ${pname(p)} 支付 1 血筹向 ${pname(imp)}【捣蛋鬼】赎回 ${reclaimed.length} 张自己的牌`);
  processPreBuyQueue(gs, now);
}
...
export function bCeoGive(gs: BloodState, playerId: string, seat: number, amount: number, now: number): void {
  ...
  p.blood -= amount;
  t.blood += amount;
  pushLog(gs, 'action', `💼 ${pname(p)}【霸道总裁】给予 ${pname(t)} ${amount} 血筹，等待其回应`);
...
export function bCeoDecide(gs: BloodState, playerId: string, accept: boolean, now: number): void {
  ...
  if (accept) {
    ...
    t.hand.push(...drawn);
    pushLog(gs, 'action', `💼 ${pname(t)} 收下 ${amount} 血筹：弃置全部手牌并抽取 ${n} 张（“不愧是顾家。”）`);
  } else {
    const pay = Math.min(amount * 2, Math.max(0, t.blood));
    t.blood -= pay;
    ceo.blood += pay;
    pushLog(gs, 'action', `💼 ${pname(t)} 拒绝：支付 ${pay} 血筹给 ${pname(ceo)}（“你顾家算什么东西。”）`);
  }
